...
from MMTCell_model import scTrans_model as create_model
...
def read_gmt(fname, sep='\t', min_g=0, max_g=5000):
    ...
    min_g and max_g are optional gene set size filters.  # gene set size == pathway size
    Args:
    ...
    return dict_pathway  # OrderedDict(['pathway_name', list['gene_name',...]], ...)
...
def fit_model(adata, gmt_path, project=None, pre_weights='', label_name='', max_g=300, max_gs=300, mask_ratio=0.015,
              n_unannotated=1, batch_size=32, embed_dim=60, depth=2, num_heads=4, lr=0.0015, lrf=0.01, epochs=100):  # # embed_dim=48
    GLOBAL_SEED = 1
    ...
    # test_acc
    acc_test_num = 0
    for i in range(len(new_adata)):
        if new_adata[i] == label_test[label_name][i]:
            acc_test_num += 1
    acc_test = acc_test_num / len(new_adata)
    print('acc_test: ' + str(acc_test_num) + '/' + str(len(new_adata)) + '=' + str(acc_test))
