...
def prediect(adata, model_weight_path, project, mask_path='', laten=False, save_att='X_att', save_lantent='X_lat',
             n_step=10000, cutoff=0.1, n_unannotated=1, batch_size=50, embed_dim=60, depth=2, num_heads=4):  # # embed_dim, cutoff
    ...
    pathway = pd.read_csv(project_path+'/pathway2.csv', index_col=0)
    ...
    gene2token = parm['feature_embed_1.fe.weight']
    ...
    gene2token.to_csv(project_path+'/gene2token_weights_1.csv')
    '''
    ...
    while n_line <= all_line:
        ...
        with torch.no_grad():
            for step, data in enumerate(data_loader):
                ...
                # np.save('./log/weights_' + str(n_batch) + '.npy', weights.detach().cpu().numpy())  # lat/weights
